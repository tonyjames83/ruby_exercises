...
# invalid input and over-constrained puzzles. This moduls also defines
...
#   refer tot he cell in the upper-left corner of the puzzle.
# - The nine 3x3 subgrids are known as "boxes" and are also numbered from 0 to 8
#   ordered from left to right and top to bottom. The box in the upper left is box 0
#   The box in the upper right is box 2. The box in the middle is box 4. The box in
#   the lower right is box 8.

# Create a new puzzle with Sudoku::Puzzle.new, specifying the initial state as a string or as an array of strings.
# The string(s) should use the characters 1 through 9 for the given values, and '.' for cells whose value is 
# unspecified. Whitespace in the input is ignored. 

# Read and write access to individual cells of the puzzle is through the [] and []= operators, which expect
# two-dimensional [row, column] indexing. These methods are numbers (not characters) 0 to 9 for cell contents.
...
# The each_unknown method is an iterator that loops through the cells of the puzzle and invokes the associated block
# once for each cell whose value in unknown.

# The possible method returns an array of integers in the range 1..9. The elements of the array are the only values allowed
# in the specified cell. If this array is empty, the the puzzle is over-specified and cannot be solved. If the array has
# only one element, then that element must be the value for that cell of the puzzle.
# 
class Puzzle
	...
	ASCII = ".123456789"
	BIN = "\000\001\002\003\004\005\0066\007\010\011"
	
	# This is the initialization method for the class. It is automatically
	# involved on new Puzzle instances created with Puzzle.new Pass the input
	# puzzle as an array of lines or as a single string. Use ASCII digits 1 to 9
	# and use the '.' character for unknown cells. Whitespace, including
	...
	def to_s
		...
		# for each row-- and collects the return value of that code into an array.
		# The code in curly braces takes a subarray of the grid representing
		# a single row and packs its numbers into a string.
	...
	def [](row, col)
		# convert two-dimensional (row,col) coordiates into a one-dimensional
		# array index then get and return the cell value at that index
		@grid[row*9 + col]
	end
	...
	# the cell at (row, col) to new value.
	...
	# It is used int he method below. The name of BoxOfIndex begins with a 
	...
	# Returns true if any row, column, or box had duplicates.
	...
	def has_duplicates?
		...
		false # If allt he tests have passed, then the board has no duplicates
	end
	...
	# at (row, col) without created a duplicate in the row, column or box.
	...
	def rowdigits(row)
		# Extract the subarray that represents the row and remove all the zeros.
		# Array subtraction is set difference, with duplicate removal.
		@grid[row*9,9] - [0]
	end
	
	# Return an array of all known values int eh specified column
	def coldigits(col)
...
# a single possible value. If it finds any, it sets their value. Single
...
# values are nil, Otherwise, the first two values returned are the row,
# and column of a cell whose value is still unknown. the third value is
# the set of values possible at that row and column. This is the minimal set
# of possible values: this is no unknown cell in the puzzle that has fewer 
# possible values. This complex return value enables a useful heuristic
# in the solve() method: that method can guess at values for cells where the
# guess is most likely to be correct.
# 
...
def Sudoku.scan(puzzle)
	...
	until changed
		...
		puzzle.each_unknown do |row, col, box|
			...
			# We care about 3 cases p.size==0, p.size==1, p.size== > 1.
			case p.size
			when 0 # No possible values means the puzzle is over-constrained
				raise Impossible
			when 1 # We've found a unique value, so set it int he grid
	...
	# Return the cell with he minimal set of possibilities.
...
# as a new puzzle object with no unknown cells. This method does not modify
# the Puzzle it is passed. Note that this method cannot detect an under-constrained
# puzzle.
def Sudoku.solve(puzzle)
	# Make a private copy of the puzzle we can modify.
	puzzle = puzzle.dup 
	...
	p.each do |guess|			# For each value in the set of possible values
		...
		begin
			...
			# raise an exception.
			return solve(puzzle)	# If it return, we just return the solution
		rescue Impossible
			next					# If it raises an exception, try the next guess
		end
	end
	
	# If we get here, then non of our guesses worked out
